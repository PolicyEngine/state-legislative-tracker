...
def upload_to_pr(chart_path: str, pr_number: int, metadata: dict, summary: dict | None = None):
    """Upload chart image to GitHub and post as an embedded PR comment.

    Strategy:
    1. Upload PNG to a dedicated 'charts' branch via GitHub Contents API
    2. Embed the raw.githubusercontent.com URL in a PR comment
    """
    ...
    try:
        ...
        print("  Error: gh CLI not available. Install GitHub CLI to upload.")
        print(f"  Chart saved locally at: {chart_path}")
        return

    repo = _get_repo_name()
    if not repo:
        print("  Could not detect GitHub repo. Saving chart locally only.")
        return

    # Upload image to 'charts' branch via GitHub Contents API
    image_url = _upload_image_to_github(chart_path, reform_id, repo)
    if not image_url:
    ...
    comment_body = (
        ...
        f"**Year**: {year}\n\n"
        f"![{state_upper} {reform_id} Impact by Earnings]({image_url})\n"
    )
    ...
    if result.returncode == 0:
        print(f"  PR comment with chart posted to #{pr_number}")
    else:
        print(f"  Failed to post PR comment: {result.stderr}")


def _upload_image_to_github(chart_path: Path, reform_id: str, repo: str) -> str | None:
    """Upload a PNG to the 'charts' branch and return the raw URL."""
    import base64

    content_b64 = base64.b64encode(chart_path.read_bytes()).decode()
    filename = f"{reform_id}.png"

    # Ensure the 'charts' branch exists
    ...
    if check.returncode != 0:
        ...
        sha_result = subprocess.run(
            ["gh", "api", f"repos/{repo}/git/ref/heads/main", "-q", ".object.sha"],
            capture_output=True, text=True,
        )
    ...
    existing = subprocess.run(
        ["gh", "api", f"repos/{repo}/contents/{filename}?ref=charts", "-q", ".sha"],
        capture_output=True, text=True,
    )
    ...
    cmd = [
        ...
        "--method", "PUT",
        "-f", f"message=Update {reform_id} household chart",
