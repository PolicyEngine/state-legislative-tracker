...
def finalize_calibration(reform_id: str, state: str = None):
    ...
    # --- Layer 2: Iterative baseline data diagnostic ---
    # If encoding is correct but gap remains, diagnose the data step by step
    kept = [h for h in history if h["status"] in ("keep", "accept")]
    final = kept[-1] if kept else None
    if final and final["pct_diff"] > 0.10:
        print(f"\n--- Layer 2: Iterative Data Diagnostic ---")
        print(f"  Reform correctly encoded but {final['pct_diff']:.0%} gap remains.")
        print(f"  Running step-by-step data diagnostic for {state.upper()}...")
        try:
            from validate_baseline import run_diagnostic_for_reform
            report = run_diagnostic_for_reform(reform_id)
            if report:
                report.print_report()

                # Add data diagnostic to diagnosis.json
                if diagnosis_path.exists():
                    with open(diagnosis_path) as f:
                        diag = json.load(f)
                    diag["data_diagnostic"] = report.to_dict()
                    factor = report.data_quality_factor
                    if abs(factor - 1.0) > 0.05:
                        direction = "higher" if factor > 1 else "lower"
                        diag["data_explanation"] = (
                            f"PE baseline for {state.upper()} runs "
                            f"{abs(factor-1)*100:.0f}% {direction} than public sources. "
                            f"Of the total {abs(report.total_gap_pct)*100:.0f}% revenue gap: "
                            f"{report.attributed_pct*100:.0f}% attributed to specific data factors, "
                            f"{report.unattributed_pct*100:.0f}% unattributed (CPS sampling/uprating)."
                        )
                    with open(diagnosis_path, "w") as f:
                        json.dump(diag, f, indent=2)
                    print(f"  Data diagnostic added to diagnosis")
        except Exception as e:
            print(f"  Data diagnostic skipped: {e}")
