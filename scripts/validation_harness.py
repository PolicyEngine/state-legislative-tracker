...
def load_corrections() -> dict:
    ...
    Tries DB first (calibration_learnings table), falls back to local file.
    These encode learned biases like "revenue-base reasoning overestimates
    by 9%" or "GA PE income base is 20% higher than state projections."
...
def _load_corrections_from_db() -> dict:
    """Load correction factors from calibration_learnings table."""
    try:
    ...
    try:
        ...
        rows = supabase.table("calibration_learnings").select(
            "strategy_name, correction_factor, state, avg_residual, residual_direction, scope, based_on_count"
        ).not_.is_("correction_factor", "null").execute()

        if not rows.data:
            # Also try state bias rows
            bias_rows = supabase.table("calibration_learnings").select(
                "state, avg_residual, residual_direction, based_on_count"
            ).not_.is_("avg_residual", "null").execute()

            if not bias_rows.data:
                return {}
            rows_data = []
            bias_data = bias_rows.data
        else:
            rows_data = rows.data
            bias_rows = supabase.table("calibration_learnings").select(
                "state, avg_residual, residual_direction, based_on_count"
            ).not_.is_("avg_residual", "null").execute()
            bias_data = bias_rows.data if bias_rows.data else []

        corrections = {"strategy_corrections": {}, "state_bias_corrections": {}}

        for row in rows_data:
            if row.get("strategy_name") and row.get("correction_factor"):
                corrections["strategy_corrections"][row["strategy_name"]] = {
                    "factor": float(row["correction_factor"]),
                    "based_on": row.get("based_on_count", 0),
                }

        for row in bias_data:
            if row.get("state") and row.get("avg_residual"):
                corrections["state_bias_corrections"][row["state"]] = {
                    "avg_residual": float(row["avg_residual"]),
                    "direction": row.get("residual_direction", ""),
                    "based_on": row.get("based_on_count", 0),
                }

        return corrections if (corrections["strategy_corrections"] or corrections["state_bias_corrections"]) else {}
